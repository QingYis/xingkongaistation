...
type Redemption struct {
	...
	PlanId       int            `json:"plan_id" gorm:"type:int;not null;default:0"`
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		case RedemptionRewardTypeSubscription:
			if redemption.PlanId <= 0 {
				return errors.New("兑换码未绑定有效订阅套餐")
			}
			var plan SubscriptionPlan
			if err := tx.First(&plan, "id = ?", redemption.PlanId).Error; err != nil {
				return errors.New("兑换码未绑定有效订阅套餐")
			}
			if !plan.Enabled {
				return errors.New("该订阅套餐未启用")
			}
